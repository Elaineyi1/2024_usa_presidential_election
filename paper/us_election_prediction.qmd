...
subtitle: "Harris Predicted to Win Popular Vote With 50.8% Popular Vote"
author: 
  - Boxuan Yi
format: 
  pdf:
...
I will only consider polls with a numeric grade of at least 2.0 that ended no more than 60 days ago, as people's voting preferences can change over time. The meaning of the numeric grade and the rationale for choosing 2.0 will be explained in @sec-data_methodology_and_measurement. The cleaning process can be found in @sec-appendix-cleaning.
...
More detailed measurements regarding YouGov can be found in @sec-appendix-methodology.

## Data Visualization and Analysis {#sec-data_data_visualization_and_analysis}
After eliminating polls conducted by pollsters with a numeric grade lower than 2, the distribution of numeric grades is shown in @fig-numeric_grade. The median grade is now about 2.75, with most polls having a grade higher than 2.5. Many state-level polls with population being 'Adults' have a grade of 3.0. Examples of pollsters with a 3.0 grade include The New York Times/Siena College, ABC News/The Washington Post, and YouGov. Examples with a 2.5 grade are the Public Policy Institute of California, Pew Research Center, and the University of Illinois Springfield Survey Research Office [@538PollsterRatings].
...
#| fig-cap: Distribution of Pollster Numeric Grades by Population After Cleaning
poll_cleaned |> 
  ggplot(aes(x = state_or_national, y = numeric_grade)) +
  geom_boxplot() + 
  geom_jitter(aes(color = population), width = 0.2, height = 0, alpha = 0.6) + 
  scale_y_continuous(breaks = seq(0, 3, by = 0.5)) +
  labs(x = "State or National Survey", y = "Numeric Grade") +
  theme_classic()
```

Since most surveys have sample sizes below 2,500, @fig-sample_size uses the logarithm of sample sizes for clearer visualization. There are significantly more state surveys than national surveys overall. Most national surveys have sample sizes under 2,500, while most state surveys are below 2,000. A few national surveys exceed 7,000, as shown by small red bumps in the distribution.
```{r}
#| message: false
#| echo: false
#| warning: false
#| label: fig-sample_size
#| fig-cap: Frequency Distribution of Log-Transformed Sample Sizes for State and National Surveys
...
The weighted average proportion of support for Harris using the national surveys is 51.3%, as indicated in @tbl-national. Consequently, the proportion of support for Trump is 48.7%, as the dataset considers only respondents who support either Harris or Trump. The difference is relatively small and will decrease when accounting for individuals who do not support either candidate.
```{r}
#| message: false
#| echo: false
#| warning: false
...
poll_harris_state_cleaned |> 
  ...
  mutate(
    Weighted_Avg_Harris_Support = round(Weighted_Avg_Harris_Support, 3)) |> 
  rename('Survey State' = state, 'Number of Survey' = Observations, 'Weighted Average Proportion of Harris Support' = Weighted_Avg_Harris_Support) |> 
  kable()
```

@fig-state uses a gradient colour scale to present a U.S. map of support by state. The bluer a state is, the higher the predicted proportion of support for Harris; the redder a state appears, the higher the predicted proportion of support for Trump. States shown in grey have no state surveys, so no state-level data is available. States on the West Coast and in the Northeast, such as Massachusetts, California, and Maryland, show high support for Harris, while states in the Midwest, including Utah, Montana, and Indiana, show strong support for Trump.
```{r}
#| message: false
#| echo: false
#| warning: false
...
\newpage
# Results {#sec-results}
The predicted national support is shown in @tbl-national_prediction. The forecast indicates that 50.8% of voters would vote for Harris, while 49.2% would vote for Trump if we only consider voters who will choose one of them.
```{r}
#| message: false
#| echo: false
#| warning: false
...
```
\newpage
@fig-state_prediction uses a gradient colour scale to indicate the predicted support rate by state. States on the West Coast and in the Northeast show a higher rate of support for Harris, with Maryland and California being the bluest. In contrast, states in the Midwest have higher support for Trump, with South Carolina appearing the reddest. Compared to @fig-state, the overall colour is lighter, indicating that the model has mitigated certain biases and preferences.
```{r}
#| message: false
#| echo: false
#| warning: false
...
## Pollsters Attract Different Audiences
...
## Polling Methodology on Voter Preferences
...
#| fig-cap: Harris's Support in Nebraska by Polling Methodology
...
If I had a budget of $100K to forecast the U.S. presidential election, I would conduct one survey now and another one after a week, both aiming for a sample size of 5,000 to 8,000 respondents for a 5-minute survey with 10 questions. I believe fewer people have the patience to finish a long survey. I would use stratified sampling to ensure that the sample reflects the U.S. population in terms of key demographics such as age, gender, race, education, region, and political affiliation.
...
- https://docs.google.com/forms/d/1-Y5sP8OanQfRrT6uXLJq9o8lg2_fefanxU9FQWxAf1c/prefill
